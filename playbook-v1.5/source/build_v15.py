...
OUT  = '/sessions/laughing-beautiful-planck/mnt/coiled-spring-backend2/playbook-v1.5/export/CoiledSpring_Playbook_v1.5.pdf'
...
def p1_cover(c):
    ...
    # v1.5: FREE PDF badge moved to top-LEFT
    c.setFillColor(ORANGE); c.circle(MG+26, H-52, 26, fill=1, stroke=0)
    c.setFillColor(white)
    c.setFont('Helvetica-Bold', 10); c.drawCentredString(MG+26, H-46, 'FREE')
    c.setFont('Helvetica-Bold', 10); c.drawCentredString(MG+26, H-58, 'PDF')
    logo(c, W/2, H-140, 48)
...
def p3_problem(c):
    ...
    # v1.5: 'The Coiled Spring Solution' in an ORANGE bordered box
    sol_h = 88
    dr(c, MG, y-sol_h, W-MG*2, sol_h, BG2, ORANGE, 1.2, 6)
    c.setFillColor(ORANGE); c.rect(MG, y-3, W-MG*2, 3, fill=1, stroke=0)
    dt(c, 'THE COILED SPRING SOLUTION', MG+14, y-17, 'Helvetica-Bold', 11, ORANGE)
    hl(c, MG+14, MG+W-MG*2-14, y-23, BORDER, 0.4)
    wt(c, 'Screen for stocks where IV Rank is below 30 — historically cheap. Enter long-dated options (90-730 DTE) before the catalyst. Let IV mean reversion work as a structural tailwind — even if price barely moves, IV expansion profits the position.',
       MG+14, y-34, W-MG*2-28, 'Helvetica', 10.5, TEXT, 15)
    y -= sol_h + 14
...
def p6_row(c):
    ...
    # v1.5: 5 sections — 2x2 grid + 1 full-width CS Score box
    ann4 = [
        ('QQQ  (Ticker)',
         'Blue-chip ETF. 50M+ daily volume. Tightest bid-ask. Ideal learning vehicle.'),
        ('PUT 620  (Strike)',
         'Long PUT profits from decline OR IV expansion. Strike 12% OTM — core Coiled Spring entry.'),
        ('336  (DTE)',
    ...
    # Full-width CS Score box — GREEN border, high emphasis
    cs_y = y - 2*interval - 12
    cs_h = 60
    dr(c, MG, cs_y-cs_h, W-MG*2, cs_h, HexColor('#0D2010'), GREEN, 1.5, 6)
    c.setFillColor(GREEN); c.rect(MG, cs_y-3, W-MG*2, 3, fill=1, stroke=0)
    c.setFillColor(GREEN); c.roundRect(MG+10, cs_y-cs_h+8, 46, 44, 6, fill=1, stroke=0)
    dt(c, '72', MG+33, cs_y-cs_h+26, 'Helvetica-Bold', 20, white, 'center')
    dt(c, 'CS', MG+33, cs_y-cs_h+12, 'Helvetica-Bold', 8,  white, 'center')
    dt(c, '72  (CS Score)', MG+68, cs_y-14, 'Helvetica-Bold', 11, GREEN)
    hl(c, MG+68, W-MG-10, cs_y-21, HexColor('#1A4020'), 0.5)
    wt(c, 'ALL THREE PILLARS CONFIRMED: HV compressed, quality high, DTE adequate.',
       MG+68, cs_y-32, W-MG*2-82, 'Helvetica-Bold', 9.5, TEXT, 14)
    c.setFillColor(GREEN); c.roundRect(W-MG-82, cs_y-cs_h+8, 72, 18, 4, fill=1, stroke=0)
    dt(c, 'VALID ENTRY', W-MG-46, cs_y-cs_h+20, 'Helvetica-Bold', 8, white, 'center')
...
def p10_portfolio(c):
    ...
    dt(c, 'Monitor every position in real time — P&L, Greeks, CS Score, and What-If access in one view',
       MG, H-80, 'Helvetica', 9.5, TEXT3)
    hl(c, 0, W, H-95, BORDER, 0.8)
    # v1.5: Context intro text — links the tracker to the What-If feature
    wt(c, 'Each position links directly to the What-If Simulator for live scenario modelling '
       'and pre-trade sizing. CS Score updates in real time as IV and DTE change.',
       MG, H-112, W-MG*2, 'Helvetica', 9.5, TEXT3, 14)
    # Stats bar
...
def p11_whatif(c):
    # v1.5: TWO separate caption boxes + restored URL
    fbg(c)
    c.setFillColor(BG2); c.rect(0, H-96, W, 90, fill=1, stroke=0)
    c.setFillColor(ORANGE); c.rect(0, H-6, W, 6, fill=1, stroke=0)
    ...
    # Layout (bottom-up):
    # micro_cta  : 72-114
    # BOX2 IV SHIFT SLIDER: 122-170
    # BOX1 WHAT THE CURVES SHOW: 178-242
    # Chart: 250 to H-112

    # ── BOX 1: WHAT THE CURVES SHOW ──
    box1_bot = 178; box1_h = 64; box1_top = box1_bot + box1_h
    dr(c, MG, box1_bot, iw, box1_h, BG2, BORDER, 0.5, 4)
    dt(c, 'WHAT THE CURVES SHOW', MG+14, box1_top-13, 'Helvetica-Bold', 9, ORANGE)
    hl(c, MG+14, MG+iw-14, box1_top-20, BORDER, 0.4)
    half2 = (iw - 28) / 2
    box1_left  = [(u'U-shape',      'long premium — both up and down moves profit'),
                  (u'Center trough','theta decay zone — flat price loses value')]
    box1_right = [(u'Curves fan down','time erosion over 600 days of DTE — choose wisely')]
    for i,(k,v) in enumerate(box1_left):
        by2 = box1_top - 30 - i*17
        dt(c, k+':', MG+14, by2, 'Helvetica-Bold', 8.5, ORANGE)
        kw = c.stringWidth(k+':', 'Helvetica-Bold', 8.5)
        dt(c, v, MG+16+kw, by2, 'Helvetica', 8.5, TEXT2)
    for i,(k,v) in enumerate(box1_right):
        by2 = box1_top - 30 - i*17
        dt(c, k+':', MG+14+half2+4, by2, 'Helvetica-Bold', 8.5, ORANGE)
        kw = c.stringWidth(k+':', 'Helvetica-Bold', 8.5)
        dt(c, v, MG+16+half2+4+kw, by2, 'Helvetica', 8.5, TEXT2)

    # ── BOX 2: IV SHIFT SLIDER ──
    box2_bot = 120; box2_h = 50; box2_top = box2_bot + box2_h
    dr(c, MG, box2_bot, iw, box2_h, BG2, BLUE, 0.8, 4)
    dt(c, 'IV SHIFT SLIDER', MG+14, box2_top-13, 'Helvetica-Bold', 9, BLUE)
    hl(c, MG+14, MG+iw-14, box2_top-20, BORDER, 0.4)
    dt(c, u'Stress-test vol collapse (−50%) or spike (+50%) before committing capital.',
       MG+14, box2_top-32, 'Helvetica', 8.5, TEXT2)
    dt(c, u'Drag the slider — all P&L curves reprice instantly. Essential check before any entry.',
       MG+14, box2_top-46, 'Helvetica', 8.5, TEXT2)

    # ── CHART ──
    chart_bot = 250; chart_top = H-112
    ...
    micro_cta(c, 'Open Portfolio → What-If tab to model positions live',
...
def p15_about(c):
    ...
    # v1.5 ORDER: Bio → Quote → Stats → Links → Disclaimer
    ...
    for i,(lb,url) in enumerate(links):
        ...
        dt(c, url, lx+10, y-28, 'Helvetica',      10,   TEXT2)
    y -= 52
...
def main():
    ...
    c.setTitle('The Coiled Spring Playbook v1.5')
