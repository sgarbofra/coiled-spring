"""
S&P 500 Calendar Monitor — Calculator
Coiled Spring Terminal

Identico alla logica ETF Calendar Calculator ma applicato ai singoli
titoli dell'S&P 500 (letti da ticker_universe.category='sp500').

Calcola per ogni ticker:
  - Maturità sintetiche costanti: 30d, 60d, 90d (interpolazione VIX-style)
  - Credit% normalizzato su spot: (price_far - price_near) / spot × 100
  - Classificazione segnale: RICH / WATCH / FAIR / CHEAP (z-score 52w)
...
# Range DTE accettabile per lo straddling
DTE_MIN_FETCH: int = 10   # < 10gg: delta esplosivi, poco rappresentativi
...
# Minimo osservazioni per z-score affidabile
MIN_HISTORY_FOR_ZSCORE: int = 20

# Sleep tra chiamate yfinance (rate limiting)
...
def _select_expirations_to_fetch(
    ...
    """Seleziona il set minimo di scadenze per coprire tutti i target DTE.

    Per ogni target trova le due scadenze reali che lo straddlano.
    Riduce le chiamate API a 3-4 per ticker (vs 10-15 disponibili).
    """
    dte_exp = [
...
def _interpolate_at_dte(
    ...
    """Interpolazione lineare a maturità costante (VIX-style).
    ...
    Returns:
        {'price': float, 'iv': float} oppure None.
    """
...
def _get_spot(t: "yf.Ticker") -> Optional[float]:
    """Estrae il prezzo spot con fallback multipli."""
    try:
...
def _fetch_chain(t: "yf.Ticker", exp: str, spot: float) -> Optional[dict]:
    ...
    Per i singoli titoli verifica anche la liquidità minima:
    - bid > 0 e ask > 0 (spread reale)
    - open_interest > 0 oppure volume > 0
    ...
    try:
        ...
        # ATM: strike più vicino allo spot
        ...
        # Liquidità minima: serve almeno bid/ask valido
        if bid > 0 and ask > 0 and ask > bid:
            mid = (bid + ask) / 2.0
            spread_pct = (ask - bid) / ((ask + bid) / 2) * 100
            # Scarta se spread > 50% del mid (opzioni illiquide)
            if spread_pct > 50:
                logger.debug(f"[SP500-CAL] spread troppo largo ({spread_pct:.1f}%) exp={exp}")
                return None
        elif last > 0:
            mid = last
        else:
        ...
        logger.debug(f"[SP500-CAL] chain fetch error exp={exp}: {exc}")
        return None


def fetch_sp500_calendar_data(ticker: str) -> Optional[dict]:
    """Fetch e calcolo maturità sintetiche 30/60/90d per un singolo titolo S&P 500.

    Identico a fetch_etf_calendar_data ma con filtro liquidità più stringente
    sullo spread bid/ask (adatto a opzioni su singoli titoli).
    ...
    try:
        ...
        if not spot or spot <= 0:
            logger.debug(f"[SP500-CAL] {ticker}: spot non disponibile")
            return None

        expirations = t.options
        if not expirations:
            logger.debug(f"[SP500-CAL] {ticker}: nessuna scadenza opzioni")
            return None
        ...
        if not to_fetch:
            logger.debug(f"[SP500-CAL] {ticker}: nessuna scadenza nel range {DTE_MIN_FETCH}-{DTE_MAX_FETCH}d")
            return None
        ...
        if not by_dte:
            logger.debug(f"[SP500-CAL] {ticker}: nessun dato ATM valido")
            return None
        ...
        logger.error(f"[SP500-CAL] {ticker}: errore fetch — {exc}")
        return None
